...
import { errorRequest, verifyRequestValue } from "./HttpRequestHelper";
...
export const findCEP = async (cep: string): Promise<IDataValue<ICep>> => {
  ...
  if (cep === undefined || cep.length < 9) {
    ...
    await apiViaCEP
      .get<ICep>(`${cep}/json/`)
      .then((resp) => {
        data = verifyRequestValue<ICep>(resp, "ERRO DE RESPOSTA CEP");
      })
      .catch((error) => {
        data = errorRequest(error, "ERRO NO SERVIDOR CEP");
      });
  }
  return data;
};
